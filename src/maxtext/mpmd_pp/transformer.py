"""Model definition for MPMD PP. Derived from train.py and layers/models.py
while dropping support for various configs."""
...
from maxtext.layers import linears, decoders
...
def loss_and_aux_from_logits(
    config,
    decoder_targets,
    decoder_targets_segmentation,
    logits,
):
    """Compute loss and auxiliary metrics from input logits."""

    one_hot_targets = jax.nn.one_hot(decoder_targets, config.vocab_size)
    xent, _ = max_utils.cross_entropy_with_logits(logits, one_hot_targets, 0.0)
    xent = nn.with_logical_constraint(
        xent, ("activation_embed_and_logits_batch_inside_vmap", "activation_length")
    )
...
class FinalLayer(nn.Module):
    """Given final hidden states, performs final normalization, LM head
    projection, and loss / aux calculation."""
    ...
    def __call__(self, y, decoder_targets, decoder_targets_segmentation, deterministic):
        ...
        activation_sharding = nn.logical_to_mesh_sharding(
            PartitionSpec(
                "activation_embed_and_logits_batch_inside_dp_vmap",
                "activation_length",
                "activation_embed",
            ),
            self.stage_mesh,
        ...
        logits_sharding = nn.logical_to_mesh_sharding(
            PartitionSpec(
                "activation_embed_and_logits_batch_inside_dp_vmap",
                "activation_length",
                "activation_vocab",
        ...
        y = models.Decoder.get_norm_layer(cfg, num_features=cfg.emb_dim)(
            ...
            kernel_axes=("norm",),
        )(y, out_sharding=activation_sharding)
        ...
        )(y, out_sharding=logits_sharding)  # We do not quantize the logits matmul.
        ...
        loss, aux = loss_and_aux_from_logits(
            cfg, decoder_targets, decoder_targets_segmentation, logits
        )
...
class PipelineParallelTransformer(nn.Module):
    ...
    def setup(self):
        cfg = self.config
        assert cfg.using_pipeline_parallelism and cfg.use_mpmd_pp
        ...
        assert len(decoder_layers) == 1, (
            f"unsupported decoder block: {cfg.decoder_block}"
        )
    ...
    def num_logical_stages(self):
        cfg = self.config
        return (
            cfg.ici_pipeline_parallelism
            * cfg.dcn_pipeline_parallelism
            * cfg.num_pipeline_repeats
        )
    ...
    def get_meshes_per_logical_stage(self):
        return [
            sharding_utils.logical_stage_idx_to_mesh(self.mesh, logical_idx)
            for logical_idx in range(self.num_logical_stages)
        ]

    def get_pipeline_stage_name(self, stage_index):
        cfg = self.config
        if not cfg.mpmd_pp_optimize_for_compile_times:
            return f"stage{stage_index}_layers"

        if stage_index == 0:
            return "stage0_layers"
        if stage_index == self.num_logical_stages - 1:
            return "stage_last_layers"
        return "stage_middle_layers"
    ...
    def get_pipeline_stage_module(self, stage_index, base_stage, num_layers_in_stage):
        cfg = self.config
        stage_mesh = sharding_utils.logical_stage_idx_to_mesh(self.mesh, stage_index)
        name = self.get_pipeline_stage_name(stage_index)
        if num_layers_in_stage == 1:
            stage_module = base_stage(
                config=cfg,
                mesh=stage_mesh,
                quant=self.quant,
                name=name,
    ...
    def _embedding(
        ...
        y = embed_as_linen(
            ...
            name="token_embedder",
            config=cfg,
            mesh=sharding_utils.logical_stage_idx_to_mesh(self.mesh, 0),
    ...
    def get_final_layer_remat_policy(self):
        assert self.config.mpmd_pp_final_layer_remat_policy != "no_remat"
        if self.config.mpmd_pp_final_layer_remat_policy == "save_logits_only":
            return jax.checkpoint_policies.save_only_these_names(
                "logits",
            )
        if self.config.mpmd_pp_final_layer_remat_policy == "full_remat":
            return jax.checkpoint_policies.save_only_these_names()
        return None
    ...
    def _stage(
        ...
        # If first stage, apply embedding.
        if stage_index == 0:
            decoder_input_tokens = y
            y = self._embedding(decoder_input_tokens, deterministic)

        ## Layers
        num_layers_per_stage, rem = divmod(
            cfg.num_decoder_layers, self.num_logical_stages
        )
        ...
        # If last stage, compute loss and aux.
        if stage_index == self.num_logical_stages - 1:
            final_layer_module = FinalLayer
            if cfg.mpmd_pp_final_layer_remat_policy != "no_remat":
            ...
            loss, aux = final_layer_module(
                config=cfg,
                stage_mesh=sharding_utils.logical_stage_idx_to_mesh(
                    self.mesh, stage_index
                ),
                name="final_layer",
            )(y, decoder_targets, decoder_targets_segmentation, deterministic)
