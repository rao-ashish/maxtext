...
class Decoder(nn.Module):
  ...
  def setup(self):
    ...
    self.decoder_layer = Decoder.get_decoder_layers(self.config)
    self.norm_layer = Decoder.get_norm_layer(self.config, num_features=self.config.emb_dim)
    if self.config.using_pipeline_parallelism:
      pipeline_stage_module = self.get_pipeline_stage_module(self.decoder_layer)
      remat_policy = Decoder.get_remat_policy(self.config)
      self.pipeline_module = pipeline.create_pipeline(
          config=self.config, mesh=self.mesh, layers=pipeline_stage_module, remat_policy=remat_policy
      )

  @staticmethod
  def minimal_policy(with_context=False, with_quantization=False):
  ...
  def get_remat_policy(cfg):
    ...
    if cfg.remat_policy != "none":
      if cfg.remat_policy in ("minimal_with_context", "minimal_flash"):
        ...
        policy = Decoder.minimal_policy(with_context=False, with_quantization=True)
        ...
        policy = Decoder.minimal_policy(with_context=True, with_quantization=True)
  ...
  def get_decoder_layers(cfg):
    ...
    match cfg.decoder_block:
      ...
      case DecoderBlockType.OLMO3:
        return [olmo3.Olmo3ScannableBlockToLinen] if cfg.scan_layers else [olmo3.Olmo3DecoderLayerToLinen]
  ...
  def get_norm_layer(cfg, num_features: int):
    ...
    if cfg.decoder_block in (
      ...
      return functools.partial(
          normalizations.Qwen3NextRMSNormLinen, num_features=num_features, shard_mode=cfg.shard_mode
      )
    else:
      raise ValueError(f"Incorrect decoder_block name {cfg.decoder_block.value=}")
  ...
  def get_pipeline_stage_module(self, decoder_blocks):
    ...
    if cfg.set_remat_policy_on_layers_per_stage:
      policy = Decoder.get_remat_policy(self.config)
  ...
  def apply_output_head(self, shared_embedding: nn.Module | nnx.Module, y, deterministic, model_mode):
    ...
    y = Decoder.get_norm_layer(self.config, num_features=y.shape[-1])(
        dtype=cfg.dtype,
        weight_dtype=cfg.weight_dtype,
        name="decoder_norm",
        epsilon=cfg.normalization_layer_epsilon,
