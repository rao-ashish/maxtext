...
def set_and_validate_pipeline_config(raw_keys):
  if using_pipeline_parallelism(raw_keys):
    use_mmpp = raw_keys["use_mmpp"]

    def modify_logical_axis_rules(logical_axis_rules):
      for idx, (name, axes) in enumerate(logical_axis_rules):
        if use_mmpp:
          if "batch" in name:
          ...
          if name == "activation_embed_and_logits_batch":
            ...
            logical_axis_rules[idx] = [name, ["stage", "data", "fsdp", "fsdp_transpose", "expert"]]
